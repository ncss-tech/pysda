...
"""
def interps(run = False):
    
    """Used for validating the interpretation passed to sda_interp:run = True for a list of interpretations:"""
...
def sda_interp(df, col, interp = None, method = 'wtd_avg',  meta=False):
    ...
    try:
        ...
        key_list = pd.Series(df[col].unique()).to_list()
        ...
        if method == 'dom_comp':
            ...
            q = """SELECT areasymbol, musym, muname, mu.mukey/1  AS MUKEY,
